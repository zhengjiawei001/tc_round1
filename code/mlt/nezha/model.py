...
class NeZhaAttention(nn.Module):
    ...
    def forward(
        ...
        outputs = (attention_output,) + self_outputs[1:]  # add attentions if we pretrain_output them
        return outputs
...
class NeZhaLayer(nn.Module):
    ...
    def forward(
        ...
        outputs = self_attention_outputs[1:]  # add self attentions if we pretrain_output attention weights
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            ...
            outputs = outputs + cross_attention_outputs[1:]  # add cross attentions if we pretrain_output attention weights
...
class NeZhaModel(NeZhaPreTrainedModel):
    ...
    Return:
        ...
        last_hidden_state (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`):
            Sequence of hidden-states at the pretrain_output of the last layer of the model.
        pooler_output (:obj:`torch.FloatTensor`: of shape :obj:`(batch_size, hidden_size)`):
            ...
            This pretrain_output is usually *not* a good summary
            of the semantic content of the input, you're often better with averaging or pooling
            the sequence of hidden-states for the whole input sequence.
        hidden_states (:obj:`tuple(torch.FloatTensor)`, `optional`, returned when ``config.output_hidden_states=True``):
            Tuple of :obj:`torch.FloatTensor` (one for the pretrain_output of the embeddings + one for the pretrain_output of each layer)
            of shape :obj:`(batch_size, sequence_length, hidden_size)`.

            Hidden-states of the model at the pretrain_output of each layer plus the initial embedding outputs.
    ...
    Examples::
        ...
        last_hidden_states = outputs[0]  # The last hidden-state is the first element of the pretrain_output tuple
...
@add_start_docstrings(
    ...
    the pooled pretrain_output) e.g. for GLUE tasks. """,
    BERT_START_DOCSTRING,
)
class NeZhaForSequenceClassification(NeZhaPreTrainedModel):
    ...
    Examples::
        ...
        cls_hidden = self.dropout(outputs[0][:, 0])
        # cls_hidden = outputs[0][:, 0]
        logits = self.classifier(cls_hidden)
        # logits = []
        # for dropout in self.dropouts:
        #     logit = self.classifier(dropout(cls_hidden))
        #     logits.append(logit)
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                # loss = loss_fct(torch.log_softmax(logits, dim=-1), labels)
...
@add_start_docstrings(
    ...
    the pooled pretrain_output and a softmax) e.g. for RocStories/SWAG tasks. """,
    BERT_START_DOCSTRING,
)
...
@add_start_docstrings(
    ...
    the hidden-states pretrain_output) e.g. for Named-Entity-Recognition (NER) tasks. """,
    BERT_START_DOCSTRING,
)
...
@add_start_docstrings(
    ...
    layers on top of the hidden-states pretrain_output to compute `span start logits` and `span end logits`). """,
    BERT_START_DOCSTRING,
)
